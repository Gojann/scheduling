...
import { addOnCallUser } from "@/store/reducer/admin/addOnCallReducer";
...
export default function Page() {
  ...
  const markOnCall = () => {
    ...
    const formData = {
      ...
      date: currentDate.toISOString(), // Convert date to string format
      // Include other form data if needed
    };

    dispatch(addOnCallUser(formData))
      .then((response) => {
        // Assuming response.data is an array of feedbacks
              ...
              <div className="mx-2">
                <div className="text-textColor text-xl md:text-2xl mb-2 px-6 py-3 flex items-center">
                  ...
                  {userDetails?.user?.firstName && (
                    <div className="">
                      <button
                        className="mx-2 bg-gray-700 rounded p-1 text-xs text-white"
                        onClick={markOnCall}
                      >
                        Mark on call
                      </button>
